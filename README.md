...
Текущая версия содержит API для работы с датасетом, алгоритм рекомендаций на основе явных правил, веб-интерфейс сотрудника и HR Dashboard. AI/LLM-модель пока не подключена. Backend обрабатывает завершение активности в памяти процесса; долговременное сохранение ещё не реализовано.
...
| HR Dashboard | Дефициты навыков, сотрудники без рекомендаций с причинами, участие по мероприятиям; фильтры по отделу и грейду |
...
frontend/
  ...
  src/
    ...
    pages/                   Список сотрудников, профиль, активность, HR Dashboard
```
...
Страница `/hr` получает все показатели одним запросом `GET /hr/overview`. Необязательные параметры `department` и `grade` ограничивают выборку во всех трёх блоках. Неизвестное значение фильтра возвращает HTTP 422. Данные рассчитываются из того же `RuntimeState` и `RecommendationEngine`, что используются в профиле и рекомендациях сотрудника.

- **Дефициты навыков:** число сотрудников с разрывом, доля среди сотрудников, для которых навык требуется на следующем грейде, средний разрыв среди имеющих дефицит и число критических дефицитов. Сортировка — по распространённости дефицита; сотрудники не ранжируются.
- **Без рекомендованного шага:** все сотрудники с пустым списком рекомендаций. Причины различаются: нет следующего грейда, требования выполнены, есть дефициты без доступных активностей. Последняя группа выделена как требующая подбора активности.
- **Участие:** все 40 мероприятий, число уникальных сотрудников, шесть статусов истории и доля завершений среди всех записей мероприятия. Отказы входят в знаменатель. Повторные попытки учитываются отдельно. Для мероприятия без записей процент не вычисляется.

Новые завершения текущего запуска добавляются как отдельные записи участия; прежние попытки не перезаписываются. Они помечены в таблице. Данные обновляются при открытии страницы, изменении фильтров и нажатии «Обновить данные». Перезапуск backend или `POST /demo/reset` удаляет новые завершения и восстанавливает исходные показатели. Исходный CSV не изменяется.

Проверка: откройте `/hr`, убедитесь, что без фильтров показаны 200 сотрудников и 2 743 записи участия. Выберите отдел и грейд, проверьте изменение трёх блоков. Завершите `EV_005` для `E0001` в исходном состоянии, вернитесь на `/hr` и нажмите «Обновить данные»: записей станет 2 744, в строке мероприятия появится одно новое завершение, дефициты System Design и API Design уменьшатся. Тесты HR находятся в `backend/tests/test_hr.py`.
...
- HR-страница показывает агрегаты исходной истории и текущего состояния; временные фильтры, экспорт отчётов и прогноз вовлечённости не реализованы.
